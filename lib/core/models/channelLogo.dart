...
class ChannelLogo {
  ...
  ChannelLogo({@required this.name, @required this.logoUrl});
  ...
  Map<String, dynamic> toMap() {
    return {"name": this.name, "logoUrl": this.logoUrl};
  }
  ...
  static ChannelLogo fromJsonMap(Map<String, dynamic> json) {
    ...
    ChannelLogo obj = new ChannelLogo(name: name, logoUrl: logoUrl);
    return obj;
  }
  ...
  static String listToJson(List<ChannelLogo> newList) {
    List<Map<String, String>> x =
        newList.map((f) => {'name': f.name, 'logoUrl': f.logoUrl}).toList();
  ...
  static Future<String> getChannelLogoUrl(
      BuildContext context, SearchVideo video) async {
    ...
    if ((config.channelLogos.singleWhere((it) => it.name == video.videoAuthor,
      ...
      return config
          .channelLogos[config.channelLogos
              .indexWhere((element) => element.name == video.videoAuthor)]
          .logoUrl;
    } else {
      ReceivePort receivePort = ReceivePort();
      await Isolate.spawn(
          ChannelLogo.getChannelLogoUrlIsolate, receivePort.sendPort);
      ...
      if ((config.channelLogos.singleWhere((it) => it.name == video.videoAuthor,
        ...
        config.addItemtoChannelLogoList(
            ChannelLogo(name: video.videoAuthor, logoUrl: url));
      }
      return url;
    }
  }
