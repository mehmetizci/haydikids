...
import 'package:haydikids/core/tagsManager.dart';
...
class DownloadInfoSet {
  ...
  Future<void> downloadMedia() async {
    ...
    if (downloadType == DownloadType.VIDEO) {
      ...
      File audioFile =
          await _downloadStream(audioStreamInfo, DownloadType.AUDIO);
      if (audioFile == null) return;
      // Path downloaded Audio file to our Video
      downloadedFile =
          await _pathAudioToVideo(downloadedFile.path, audioFile.path);
      ...
      downloadedFile =
          await ffmpegConverter.clearFileMetadata(downloadedFile.path);
      if (audioModifiers.volume != 1.0 ||
          audioModifiers.bassGain != 0 ||
          audioModifiers.trebleGain != 0) {
        currentAction.add(language.labelPatchingAudio);
        downloadedFile = await ffmpegConverter.applyAudioModifiers(
            downloadedFile.path, audioModifiers);
      }
      if (downloadedFile == null) return;
      // Check if Conversion is needed
      if (await ffmpegConverter.audioConversionRequired(
          convertFormat, downloadedFile.path)) {
        downloadedFile =
            await _convertAudio(convertFormat, downloadedFile.path);
        if (downloadedFile == null) return;
      }
    }
    // Rename File
    ...
    Permission.storage.request().then((value) async {
      if (value == PermissionStatus.granted) {
        ...
        File finalFile;
        try {
          finalFile = await downloadedFile
              .copy("$downloadPath/${metadata.title}.$format");
  ...
  Future<File> _downloadStream(
      StreamInfo streamToDownload, DownloadType type) async {
    // Download
    File download = File((await getTemporaryDirectory()).path +
        "/" +
    ...
    if (streamToDownload != null) {
      ...
      while (retryCount < 3) {
        try {
          audioManifest = await yt.videos.streamsClient
              .getManifest(videoDetails.id)
  ...
  Future<File> renameFile(File file, String newName) async {
    String filePath = file.path.replaceAll("/${file.path.split('/').last}", '');
    ...
    try {
      ...
      if (convertFormat == FFmpegActionType.ConvertToAAC) {
        ...
        if (isURL(metadata.coverurl)) {
          ...
          try {
            response = await http
                .get(metadata.coverurl)
                .timeout(Duration(seconds: 10));
            await artwork.writeAsBytes(response.bodyBytes);
            var decodedImage =
                await decodeImageFromList(artwork.readAsBytesSync());
          ...
          if (response == null || response.bodyBytes == null) {
            try {
              response = await http
                  .get(videoDetails.thumbnails.mediumResUrl)
                  .timeout(Duration(seconds: 10));
              await artwork.writeAsBytes(response.bodyBytes);
          ...
          croppedImage =
              await NativeMethod.cropToSquare(File(metadata.coverurl));
        }
        await TagsManager.writeArtwork(
            songPath: filePath, artworkPath: croppedImage.path);
        // Copy our CoverArt to default folder
        await croppedImage.copy(
  ...
  Future<void> finishDownload(File finalFile) async {
    ...
    await dbHelper.insertDownload(new SongFile.toDatabase(
        ...
        downloadType: downloadType == DownloadType.AUDIO ? "Audio" : "Video",
